...
public class HandPoker extends Hand {
	...
	public boolean isStraightFlush() {
		...
		if (isFlush() && isStraight()) {
			this.HSP.seteHandStrength(eHandStrength.StraightFlush);
	...
	public boolean isFlush() {
		...
		eSuit eSuitOfFirstCard = super.getCards().get(0).geteSuit();

		for(int i = 1; i< iCardCnt; i++) {
			if(super.getCards().get(i).geteSuit()!=eSuitOfFirstCard) {
				bisFlush = false;
				break;
			}
		}
